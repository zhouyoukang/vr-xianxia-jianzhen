...
### 8.2 待增强 🔧
- **渐进加载**: low → medium 自动升级 (已有自动画质逻辑，可优化)
- **SPZ 缓存**: 实现类 SplatCache 的内存管理
- **MPI 背景**: 使用 MPI URL 作为增强型全景背景
- **动态场景发现**: 当有 API token 时，实时搜索/浏览 Marble 场景
- **场景预加载**: 预加载相邻场景的 low-res SPZ
- **碰撞网格**: 利用 collider_mesh 实现地面检测
...
| HUD缓存状态显示 | ✅ | `缓N` 实时显示 |
| MPI资源 | ❌ | CDN返回404，暂不可用 |
| WASM排序管线 | 🔶 | SparkJS内置，已确认API但未独立提取 |
| 注视点渲染 | 🔶 | API已解构，VR模式可未来集成 |
| 全景截图 | 🔶 | 着色器已提取，可移植 |
